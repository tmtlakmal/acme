...
class AgentHandler():
    ...
    def create_loop(self, id, trained : bool = False):
        self.env_loops.append(self.create_env_loop(id, trained))
    ...
    def create_env_loop(self, id, trained = False):
        train_summary_writer = self.createTensorboardWriter("./train/", "DQN")

        discounts = [1, 1, 0.9]
        extension = array_to_string(discounts)
        if trained:
            env = self.make_environment(id, env=self.env, front_vehicle=False, extension=extension)
        ...
        env = self.make_environment(id, env=self.env, extension=extension)
        agent = self.create_agent(env, discounts, train_summary_writer)
    ...
    def make_environment(self, id=1, env=None,  multi_objective=True, front_vehicle=True, extension='') -> dm_env.Environment:
    ...
    def create_agent(self, env : dm_env, discounts : [int], train_summary_writer : None, trained : bool = False):
