...
class TLDQNLearner(acme.Learner, tf2_savers.TFSaveable):
  ...
  def __init__(
    ...
    # Learner state.
    self._variables: List[List[tf.Tensor]] = [network.trainable_variables for network in networks]
    ...
    if checkpoint:
      ...
      for index, network in enumerate(networks):
        vars['network' + str(index)] = network
      self._snapshotter = tf2_savers.Snapshotter(
        objects_to_save=vars, time_delta_minutes=60.)
    else:
